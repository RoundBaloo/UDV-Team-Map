...
class SyncRecord(Base):
    ...
    job: Mapped["SyncJob"] = relationship(back_populates="records")
    decided_by: Mapped[Optional["Employee"]] = relationship()

    __table_args__ = (
        CheckConstraint(
            "entity_type IN ('employee','team','org_unit')", name="ck_sync_record_entity_type"
        ),
        CheckConstraint(
            "action IN ('create','update','verify','archive')", name="ck_sync_record_action"
        ),
        CheckConstraint(
            "status IN ('applied','orphaned','error')", name="ck_sync_record_status"
        ),
        CheckConstraint(
